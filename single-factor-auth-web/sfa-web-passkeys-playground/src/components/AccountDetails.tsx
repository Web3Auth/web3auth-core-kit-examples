...
function AccountDetails({ children }: AccountDetailsProps) {
  ...
  return (
    <div className="w-11/12 px-4 sm:px-6 lg:px-8  z-0">
      ...
      <div className="md:p-8 p-4 mt-6 mb-0 space-y-1 sm:space-y-4 rounded-lg bg-white overflow-hidden flex flex-col items-center justify-center">
        <span className="text-md sm:text-xl md:text-2xl text-gray-800 font-bold w-fit">{user?.name as string}</span>
        <div
          className="w-fit text-[8px] sm:text-sm bg-gray-100 text-gray-600 pl-1 pr-1 sm:p-1 sm:pl-3 sm:pr-3 rounded-full z-0 flex flex-row justify-between space-x-2 sm:space-x-4 items-center cursor-pointer overflow-scroll"
          onClick={() => {
